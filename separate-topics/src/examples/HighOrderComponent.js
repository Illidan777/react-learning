...
// if there are a lot of modifications of  HOC - bad practice to use it.
// It is better remove it and put logic in dif components
const withSlider = (BaseComponent, getData) => {
    return (props) => {
        ...
        // if so many props here so less sense to use HOC (cause components are so different)
...
function HighOrderComponentTest() {
    return (
        <>
            <Hello/>
            <HelloWithLogger/>
            <SliderWithFirstFetch name={'some extra prop'}/>
            <SliderWithSecondFetch/>
        </>
    );
}

export default HighOrderComponentTest;
